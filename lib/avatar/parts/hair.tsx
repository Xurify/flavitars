...
export const HairBack: Record<string, PartComponent> = {
  bald: () => null,
  bobCutSharp: ({ fill }) => (
    <g>
      <path
  ...
  shortCurlyBob: ({ fill }) => {
    ...
    return (
      <g>
        ...
        <path
          d="M 18 25 C 5 40, 0 70, 15 95 L 85 95 C 100 70, 95 40, 82 25 Z"
          fill={hairColor}
          stroke="black"
          strokeWidth="1.5"
        />
...
export const HairFront: Record<string, PartComponent> = {
  bald: () => null,
  bobCutSharp: ({ fill, hatId }) => {
  ...
  shavedSidesLongBack: ({ fill, hatId }) => {
    const isFullHat = hatId && hatId !== "none" && !SMALL_HATS.includes(hatId);
    const hairColor = fill || "var(--avatar-hair, #000)";
    return (
      <g>
  ...
  spikyMohawk: ({ fill, headId, hairId, hatId }) =>
    hatId && hatId !== "none" && !SMALL_HATS.includes(hatId) ? null : (
  ...
  singleTopKnot: ({ fill, headId, hairId }) => (
    <g transform={getHeadHairTransform(headId, hairId, -1)}>
      <path
  ...
  doubleSpaceBuns: ({ fill, headId, hairId }) => (
    <g transform={getHeadHairTransform(headId, hairId, -1)}>
      <path
  ...
  buzzCut: ({ fill, headId, hatId, hairId }) =>
  ...
  lowPonytail: ({ fill, headId, hairId }) => (
    <g transform={getHeadHairTransform(headId, hairId, -1)}>
      <path
  ...
  // In hair.tsx -> HairFront object
  shortCurlyBob: ({ fill, hatId }) => {
    const hairColor = fill || "var(--avatar-hair, #E8C872)";
    const isFullHat = hatId && hatId !== "none" && !SMALL_HATS.includes(hatId);
    return (
      <g>
